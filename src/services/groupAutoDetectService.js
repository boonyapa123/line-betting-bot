...
class GroupAutoDetectService {
   ...
   */
  static saveGroupLocally(groupId, groupName, timestamp) {
    try {
   ...
   * Remove group from local storage
   */
  static removeGroupLocally(groupId) {
    try {
      if (!fs.existsSync(GROUPS_FILE)) {
        return;
      }
   ...
   */
  static async updateEnvFile(groupId) {
    try {
...
✅ Group ID ได้ถูกบันทึกแล้ว`;
   ...
   * Record group to Google Sheets
   */
  static async recordGroupToSheets(groupId, groupName, timestamp) {
    try {
...
module.exports = { GroupAutoDetectService };
