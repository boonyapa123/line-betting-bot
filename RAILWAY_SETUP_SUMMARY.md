# Railway.app Setup - สรุปขั้นตอน

## ✅ ที่ฉันทำให้คุณแล้ว

1. ✅ สร้าง `Procfile` - บอก Railway ว่าต้องรัน `node src/index.js`
2. ✅ สร้าง `.railway.json` - ตั้งค่า Railway deployment
3. ✅ สร้าง `src/config/googleCredentials.js` - Helper function อ่าน credentials จาก environment variable
4. ✅ ปรับแต่ง `src/services/googleSheetsDatabaseService.js` - ใช้ helper ใหม่
5. ✅ ปรับแต่ง `src/services/googleSheetsService.js` - ใช้ helper ใหม่
6. ✅ ปรับแต่ง `src/services/openBettingRecordService.js` - ใช้ helper ใหม่

---

## 🚀 ขั้นตอนที่คุณต้องทำ

### ขั้นตอนที่ 1: Push Code ไปยัง GitHub

```bash
git add .
git commit -m "Setup Railway deployment with Google credentials support"
git push origin main
```

### ขั้นตอนที่ 2: สมัคร Railway.app

1. ไปที่ https://railway.app
2. Click "Start Project"
3. Login ด้วย GitHub account
4. Authorize Railway

### ขั้นตอนที่ 3: Deploy ไปยัง Railway

1. ใน Railway Dashboard click "New Project"
2. เลือก "Deploy from GitHub repo"
3. เลือก repository ของคุณ
4. Railway จะ auto-detect Node.js project

### ขั้นตอนที่ 4: ตั้ง Environment Variables

ใน Railway Dashboard > Variables เพิ่ม:

```
LINE_CHANNEL_ACCESS_TOKEN=your_token_here
LINE_CHANNEL_SECRET=your_secret_here
GOOGLE_SHEETS_ID=your_sheets_id_here
GOOGLE_CREDENTIALS_BASE64=
LIFF_ID=your_liff_id_here
PORT=3000
NODE_ENV=production
```

### ขั้นตอนที่ 5: ได้ Public URL

1. ใน Railway Dashboard ไปที่ Deployments
2. ดู "Public URL" (เช่น `https://line-betting-bot-production.up.railway.app`)

### ขั้นตอนที่ 6: ตั้ง Webhook URL ใน LINE

1. ไปที่ https://developers.line.biz/console/
2. เลือก Channel ของคุณ
3. ไปที่ "Messaging API" settings
4. ตั้ง "Webhook URL" เป็น:
   ```
   https://your-railway-url/webhook
   ```
5. Enable "Use webhook"
6. Click "Verify"

### ขั้นตอนที่ 7: ทดสอบ

```bash
# ทดสอบ health check
curl https://your-railway-url/health

# ทดสอบใน LINE
# พิมพ์ "สรุปยอดแทง"
```

---

## 📊 ตรวจสอบ Logs

ใน Railway Dashboard:
1. ไปที่ Deployments
2. Click ที่ deployment ล่าสุด
3. ดู Logs

---

## 🔄 Auto-Deploy

Railway จะ auto-deploy เมื่อ push ไปยัง GitHub:

```bash
git add .
git commit -m "Update bot"
git push origin main
# Railway จะ auto-deploy ใน 1-2 นาที
```

---

## 💰 Pricing

- **Free Tier**: $5/เดือน (เพียงพอสำหรับ bot)
- **Pro Tier**: $20/เดือน

---

## 🆘 Troubleshooting

### ปัญหา: Deployment Failed
- ตรวจสอบ Logs ใน Railway Dashboard
- ตรวจสอบ package.json มี `start` script
- ตรวจสอบ PORT ใช้ environment variable

### ปัญหา: Webhook ไม่ทำงาน
- ตรวจสอบ Webhook URL ใน LINE Developers Console
- ตรวจสอบ LINE_CHANNEL_SECRET ถูกต้อง
- ทดสอบ `/health` endpoint

### ปัญหา: ไม่สามารถเชื่อมต่อ Google Sheets
- ตรวจสอบ GOOGLE_CREDENTIALS_BASE64 ถูกต้อง
- ตรวจสอบ GOOGLE_SHEETS_ID ถูกต้อง
- ดู Logs ใน Railway Dashboard

---

## ✅ สรุป

✅ Code ปรับแต่งแล้ว
✅ Procfile สร้างแล้ว
✅ Helper function สร้างแล้ว
✅ พร้อม deploy ไปยัง Railway
✅ ทำงาน 24/7 โดยอัตโนมัติ
✅ Auto-deploy เมื่อ push code
