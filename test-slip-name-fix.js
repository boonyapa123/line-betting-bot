/**
 * Test Slip Name Fix
 * ตรวจสอบว่า slip name ถูกบันทึกอย่างถูกต้องเมื่อ reply
 */

require('dotenv').config();

const { google } = require('googleapis');
const fs = require('fs');
const path = require('path');

// Load credentials
const credentialsPath = path.join(__dirname, 'credentials.json');
const credentials = JSON.parse(fs.readFileSync(credentialsPath, 'utf8'));

const auth = new google.auth.GoogleAuth({
  credentials,
  scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
});

const sheets = google.sheets({ version: 'v4', auth });

async function testSlipNameFix() {
  try {
    console.log('📊 Fetching Bets sheet data...\n');

    // ดึงข้อมูลจากชีท Bets
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: 'Bets!A:U',
    });

    const rows = response.data.values || [];
    console.log(`📈 Total rows: ${rows.length}\n`);

    // ข้ามแถวหัวตาราง
    const dataRows = rows.slice(1);

    // ตรวจสอบแถวที่มี User B (จับคู่สำเร็จ)
    console.log('🔍 Checking matched bets (with User B)...\n');

    let issueCount = 0;
    let correctCount = 0;

    dataRows.forEach((row, index) => {
      const rowNum = index + 2; // +2 เพราะ +1 สำหรับ header, +1 สำหรับ 1-indexed
      
      // Column L (index 11) = USER_B_NAME
      const userBName = row[11] || '';
      
      // ถ้ามี User B แสดงว่าจับคู่สำเร็จ
      if (userBName) {
        const userAName = row[2] || 'Unknown';
        const messageA = row[3] || '';
        const slipName = row[4] || '';
        const sideA = row[5] || '';
        const amount = row[6] || '';
        const sideB = row[12] || '';
        const amountB = row[7] || '';
        
        // ตรวจสอบว่า slip name มีรูปแบบ "ราคา ชื่อบั้งไฟ" (ผิด)
        if (slipName && slipName.match(/^\d+-\d+\s+/)) {
          issueCount++;
          console.log(`❌ Row ${rowNum}: Slip name has price prefix`);
          console.log(`   Slip Name: "${slipName}"`);
          console.log(`   User A: ${userAName} (${sideA}) ${amount} บาท`);
          console.log(`   User B: ${userBName} (${sideB}) ${amountB} บาท`);
          console.log(`   Message A: ${messageA}`);
          console.log('');
        } else {
          correctCount++;
          console.log(`✅ Row ${rowNum}: Slip name is correct`);
          console.log(`   Slip Name: "${slipName}"`);
          console.log(`   User A: ${userAName} (${sideA}) ${amount} บาท`);
          console.log(`   User B: ${userBName} (${sideB}) ${amountB} บาท`);
          console.log('');
        }
      }
    });

    console.log(`\n📊 Summary:`);
    console.log(`   Matched bets checked: ${correctCount + issueCount}`);
    console.log(`   ✅ Correct slip names: ${correctCount}`);
    console.log(`   ❌ Issues found: ${issueCount}`);

    if (issueCount === 0) {
      console.log('\n🎉 All matched bets have correct slip names!');
    } else {
      console.log(`\n⚠️  Found ${issueCount} matched bets with incorrect slip names`);
    }

  } catch (error) {
    console.error('❌ Error:', error.message);
  }
}

testSlipNameFix();
